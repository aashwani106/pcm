...
export default function StudentCalendarScreen() {
  ...
  const holidaysSet = useMemo(() => new Set(data?.holidays ?? []), [data?.holidays]);
  const attendanceByDate = data?.attendance_by_date ?? {};
            ...
            <View style={styles.grid}>
              {cells.map((cell, idx) => {
                ...
                return (
                  <Pressable
                    key={cell.dateISO}
                    onPress={() => setSelectedDate(cell.dateISO)}
                    style={[styles.dayCell, dayType === 'present' && styles.dayPresent, dayType === 'absent' && styles.dayAbsent, dayType === 'holiday' && styles.dayHoliday, dayType === 'future' && styles.dayFuture]}
                  >
                    <Text style={[styles.dayText, dayType === 'future' ? styles.dayTextFuture : styles.dayTextSolid]}>
                      {cell.dayNumber}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  monthNav: { marginTop: Spacing.md, marginBottom: Spacing.sm, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'rgba(255,255,255,0.6)', borderWidth: 1, borderColor: Colors.border, borderRadius: BorderRadius.lg, paddingHorizontal: Spacing.sm, paddingVertical: Spacing.xs },
